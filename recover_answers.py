...
def recover_answers():
    ...
    for sid in subagent_ids:
        transcript_path = os.path.join(brain_dir, sid, ".system_generated", "logs", "transcript_full.jsonl")
                            ...
                            if call.get('name') == 'write_to_file':
                                ...
                                if code_content.strip().startswith('['):
                                    try:
                                        ...
                                        if isinstance(chunk_data, list) and len(chunk_data) > 0 and 'answer' in chunk_data[0]:
