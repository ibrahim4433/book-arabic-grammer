...
This document contains the aggregated documentation of every usable and fixable tool in the project, organized by directory.
...
## docs/Archive/C3/plus/C3/C3_Lessons_Text/old
...
## system-workspace/tools/automation
...
## system-workspace/tools/automation/modules
...
## system-workspace/tools/new-tools
...
- **Status:** Usable
- **Purpose:** Cleans OCR noise (headers, footers, phone numbers, page numbers) from a raw text file using regex.
- **Inputs:** `system-workspace/text-data/raw/full-book.txt`
- **Outputs:** `system-workspace/text-data/raw/full-book-cleaned.txt`
- **Usage:** `python3 system-workspace/tools/new-tools/clean_raw_book.py`
- **Workflow Integration:** Initial data preparation workflow before the 1-Page-Plan generation.

### `system-workspace/tools/new-tools/debug_ocr.py`
- **Status:** Usable
- **Purpose:** A Gradio web UI tool for visually debugging OCR results with Tesseract, applying binarization and contrast adjustments.
- **Inputs:** User-uploaded images or PDFs (interactive).
- **Outputs:** Interactive Gradio UI (images and extracted text).
- **Usage:** `python3 system-workspace/tools/new-tools/debug_ocr.py`
- **Workflow Integration:** General utility for OCR debugging.

### `system-workspace/tools/new-tools/fill_missing_bs4.py`
- **Status:** Usable
- **Purpose:** Updates answers in the `98.00_p120_Answers.html` file using BeautifulSoup to match missing answers with a predefined dictionary.
- **Inputs:** `pages/98.00_p120_Answers.html`
- **Outputs:** Modifies `pages/98.00_p120_Answers.html`
- **Usage:** `python system-workspace/tools/new-tools/fill_missing_bs4.py`
- **Workflow Integration:** Part of the post-processing tools to correct and fill in missing information in answer pages.

### `system-workspace/tools/new-tools/fix_toc_styles.py`
- **Status:** Usable
- **Purpose:** Generates Table of Contents HTML pages (`00.X_TOC.html`) by scanning generated HTML files and extracting titles/numbers.
- **Inputs:** All HTML files in `pages/` (excluding templates).
- **Outputs:** Creates/overwrites `pages/00.X_TOC.html` files.
- **Usage:** `python3 system-workspace/tools/new-tools/fix_toc_styles.py`
- **Workflow Integration:** General generation workflow.
...
- **Status:** Usable
- **Purpose:** A rich terminal UI dashboard for orchestrating the overall Arabic Grammar Book generation workflow.
- **Inputs:** Various project configurations and state.
- **Outputs:** Displays a live dashboard with logs, progress, and status.
- **Usage:** `python system-workspace/tools/new-tools/system.py`
- **Workflow Integration:** Acts as the main control room for managing the entire generation process, bridging the different modules and providing visibility into the workflow.

### `system-workspace/tools/new-tools/test_get.py`
- **Status:** Usable
- **Purpose:** Testing the `TextProcessor`'s `get_lesson_number` function.
- **Inputs:** `input/TOC.json`
- **Outputs:** Standard output (prints lesson number).
- **Usage:** `python3 system-workspace/tools/new-tools/test_get.py`
- **Workflow Integration:** General text processing testing.

## system-workspace/tools/new-tools/new-beta-page-maker
...
- **Status:** Usable
- **Purpose:** A simple script to test downloading the audio of a YouTube video using the `pytubefix` library.
- **Inputs:** A YouTube video URL (hardcoded)
- **Outputs:** `test_audio.mp3`
