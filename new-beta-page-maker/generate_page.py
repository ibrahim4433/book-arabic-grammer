...
TEMPLATE_BLOCK = re.sub(r'<p[^>]*>\s*\[CONTENT_TEXT\]\s*</p>', '[CONTENT_TEXT]', TEMPLATE_BLOCK, flags=re.DOTALL)
...
rows_data3 = [
    ('<span class="font-bold text-primary">الزِّيَادَةُ (تُكْتَبُ وَلَا تُلْفَظُ)</span>', 'بَعْدَ وَاوِ الْجَمَاعَةِ، تَنْوِينِ النَّصْبِ، إِطْلَاقِ الشِّعْرِ، فِي (عَمْرو، أُولَئِكَ)', '<span class="highlight-red">سَافَرُوا</span>، <span class="highlight-red">كِتَابًا</span>، <span class="highlight-red">عَمْرو</span>'),
    ('<span class="font-bold text-accent">الْحَذْفُ (تُلْفَظُ وَلَا تُكْتَبُ)</span>', 'أَلِفُ (اللَّهُ، الرَّحْمَنُ)، (مَا) الِاسْتِفْهَامِيَّةُ، (نُونُ) مِنْ/عَنْ، لَامُ الَّذِي', '<span class="highlight-red">اللَّهُ</span>، <span class="highlight-red">لِمَ؟</span>، <span class="highlight-red">الَّذِي</span>')
]
rows_html3 = ""
for r in rows_data3:
    row = TEMPLATE_TABLE_ROW.replace('[CELL_1]', r[0]).replace('[CELL_2]', r[1]).replace('[CELL_3]', r[2])
    rows_html3 += row
...
rows_data8 = [
    ('<span class="font-bold text-primary">لَامٌ وَاحِدَةٌ مُشَدَّدَةٌ</span>', 'لِلْمُفْرَدِ بِنَوْعَيْهِ، وَجَمْعِ الْمُذَكَّرِ', '<span class="highlight-blue">الَّذِي</span>، <span class="highlight-blue">الَّتِي</span>، <span class="highlight-blue">الَّذِينَ</span>'),
    ('<span class="font-bold text-primary">لَامَانِ اثْنَتَانِ</span>', 'لِلْمُثَنَّى بِنَوْعَيْهِ، وَجَمْعِ الْمُؤَنَّثِ', '<span class="highlight-blue">اللَّذَانِ</span>، <span class="highlight-blue">اللَّتَانِ</span>، <span class="highlight-blue">اللَّوَاتِي</span>، <span class="highlight-blue">اللَّائِي</span>'),
    ('<span class="font-bold text-primary">دُخُولُ لَامِ الْجَرِّ</span>', 'تُكْتَبُ بِلَامَيْنِ عِنْدَ دُخُولِ اللَّامِ عَلَيْهَا', '<span class="highlight-blue">لِلَّذِينَ</span>، <span class="highlight-blue">لِلَّذِي</span>، <span class="highlight-blue">لِلَّتَيْنِ</span>')
]
rows_html8 = ""
for r in rows_data8:
    row = TEMPLATE_TABLE_ROW.replace('[CELL_1]', r[0]).replace('[CELL_2]', r[1]).replace('[CELL_3]', r[2])
...
block10_wrapped = re.sub(r'<div class="benefit-box">.*?</div>', '', block10_wrapped, flags=re.DOTALL)
...
def inject_ids(html_content):
    ...
    html_content = re.sub(r'<section[^>]*class="[^"]*\bcontent-block\b[^"]*"[^>]*>', replace_func, html_content)
    html_content = re.sub(r'<div[^>]*class="[^"]*\bcontent-block\b[^"]*"[^>]*>', replace_func, html_content)
    return html_content
