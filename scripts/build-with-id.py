#!/usr/bin/env python3
"""build-with-id.py — Arabic Grammar Book Debug PDF Builder using Playwright.

Compiles all pages in /pages/ into a single A4 PDF with visible ID badges
attached to every element with an 'id' attribute (for visual tracing & debugging).
Uses Playwright for pixel-perfect Chrome rendering matching build.py.

Usage:
    uv run python scripts/build-with-id.py
    uv run python scripts/build-with-id.py --output output/book-debug.pdf
    uv run python scripts/build-with-id.py --pages-dir pages/ --dry-run
"""
...
try:
    ...
    print("❌ Error: pypdf is not installed. Run: uv add pypdf", file=sys.stderr)
    sys.exit(1)
...
DEBUG_ID_STYLE = """
<style>
/* DEBUG CSS for ID Visualization */
*[id] {
...
class BuildConfig:
    ...
    output_pdf: Path = PROJECT_ROOT / "output/book-debug.pdf"
...
def build_book(config: BuildConfig) -> BuildResult:
    ...
    print(f"📄 Found {len(pages)} page(s) in '{config.pages_dir}' (Debug Mode with IDs)")
    ...
    temp_dir = tempfile.mkdtemp(prefix="book_build_debug_")
    pdf_parts = []

    try:
        chrome_exe = config.chrome_executable or _find_system_chrome()

        print("\n🖨  Rendering debug pages via Chrome with visual IDs...")
        if chrome_exe:
        ...
        with sync_playwright() as p:
            ...
            # 2. Content Pages with ID Badges
            for i, page_file in enumerate(pages):
                print(f"  ⚙  Rendering {page_file.name} ({i+1}/{len(pages)})...")
                try:
                    content = page_file.read_text(encoding="utf-8")

                    # Inject debug ID styling
                    if "</head>" in content:
                        content = content.replace("</head>", f"{DEBUG_ID_STYLE}\n</head>")
                    else:
                        content = DEBUG_ID_STYLE + "\n" + content
        ...
        <span class="watermark-text" style="font-family: 'Noto Kufi Arabic'; font-weight: 900; font-size: 80pt; color: #000000; opacity: 0.4; transform: rotate(-45deg); white-space: nowrap;">{config.watermark_text}</span>
                    ...
                    # Inject global background if not already present
                    ...
                    # Save modified content to a temp file in the same directory so relative links resolve
                    temp_page = page_file.with_name(f"_tmp_render_debug_{page_file.name}")
                    try:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="build-with-id.py",
        description="Compile Arabic Grammar book pages into a Debug PDF with visible ID badges using Playwright.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("--pages-dir", type=Path, default=PROJECT_ROOT / "pages", metavar="DIR")
    parser.add_argument("--output", type=Path, default=PROJECT_ROOT / "output/book-debug.pdf", metavar="PATH")
...
def main() -> None:
    ...
    if args.install_browsers:
        ...
        print("✅ Done! You can now run: uv run python scripts/build-with-id.py")
        return
    ...
    if result.output_path:
        ...
        print(
            f"\n✅ Debug PDF generated: {result.output_path} "
