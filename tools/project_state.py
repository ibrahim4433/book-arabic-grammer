...
class ProjectState:
    ...
    def extract_metadata(self, filepath):
        ...
        headers = re.findall(r'<div class="block-header[^"]*">\s*<span[^>]*>([^<]+)</span>', content)
        if headers:
    ...
    def verify(self, filepath):
        ...
        if new_num == stored_num and new_num != "":
            if new_title != stored_title:
                issues.append(f"Title Mismatch: Expected '{stored_title}' (from active lesson {stored_num}), found '{new_title}'.")
