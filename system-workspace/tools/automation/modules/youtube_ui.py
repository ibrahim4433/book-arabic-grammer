...
class YouTubeOfflineUI:
    ...
    def generate_ui_table(self):
        table = Table(title="YouTube Transcription Progress (Max 3 Concurrent)", box=box.ROUNDED, expand=True)
        ...
        with self.lock:
            ...
            for seq in sorted(self.tasks_status.keys()):
                ...
                status_color = "yellow"
                if t["status"] == "SUCCESS": status_color = "green"
                elif t["status"] == "ERROR": status_color = "red"
                elif t["status"] == "PENDING": status_color = "dim"
    ...
    def process_batch(self, videos):
        ...
        console.print(f"[bold green]✅ Batch Completed in {int(time.time() - start_time)} seconds![/bold green]")
...
def run_jules_youtube_ui(state_manager=None):
    ...
    mode = questionary.select(
        "Select Mode:",
        choices=[
            "1) Process Single YouTube Link",
            "2) Batch Process from CSV file",
            "3) Cancel"
        ]
    ).ask()
    ...
    if mode.startswith("1"):
        ...
        if not title: title = "Single_Video"
        ...
        # Find CSV files
        csv_dirs = [
            project_root / "input" / "csv-youtube",
            project_root / "Pdf-new-resource"
        ]
        ...
        if not csv_files:
            console.print("[red]No CSV files found in input/csv-youtube/ or Pdf-new-resource/.[/red]")
            time.sleep(2)
            return
