    ...
    def process_plan(self, plan_path, callback=None):
        ...
        if not session_id:
            ...
            tools_and_docs = (
                ...
                f"\n"
                f"**MANDATORY TOOL EXECUTION SEQUENCE:**\n"
                f"You must execute your tasks in this exact order. **CRITICAL: Do NOT invent new scripts. Do NOT write Playwright tests. Do NOT use `node`. Do NOT use `frontend_verification_complete`.**\n"
                f"1. **Explore Templates:** Use your file reading tools to read the required `Jules-workspace/Templates/` so you don't hallucinate HTML structures.\n"
                f"2. **Create HTML:** Write your generated HTML to the `pages/` directory using a bash heredoc.\n"
                f"3. **Dummy Images (If applicable):** If you used `TEMPLATE_C_POET_BIO`, create the dummy image using `touch input/integrated-pictures/pic_[PAGE_NUMBER].jpg`.\n"
                ...
                f"   `python3 verify_plan.py plans/<plan_filename>.md`\n"
                f"8. **Clean up:** Delete any temporary python or markdown scratch files you created during exploration.\n"
            )
            if elements_text:
