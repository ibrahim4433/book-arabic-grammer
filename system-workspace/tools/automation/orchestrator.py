...
class ArchitectGEM:
    ...
    def generate_plan(self, system_prompt_path, user_content_str, project_state_str):
        ...
        command = ["gemini", "--non-interactive", "--model", self.model, "--output-format", "text"]
        ...
        try:
            ...
            print(
                "❌ Error: 'gemini' command not found. Did you run 'npm install -g @google/gemini-cli'?"
            )
...
def main():
    ...
    parser.add_argument(
        "--repo", default="ibrahim4433/book-arabic-grammer", help="GitHub repo name for Jules"
    )
    ...
    if args.lesson:
        ...
        if not current_lesson.exists():
            ...
            sys.exit(1)
        with open(current_lesson, encoding="utf-8") as f:
            user_content = f.read()
        ...
        # Find images
        images = sorted(
            list(img_dir.glob("*.jpg")) + list(img_dir.glob("*.png")) + list(img_dir.glob("*.webp"))
        )
        if not images:
