...
def extract_classes(content):
    ...
    for line in content.splitlines():
        if "class=" in line:
            m = re.search("class=" + chr(34) + "(.*?)" + chr(34), line) or re.search(
                "class=" + chr(39) + "(.*?)" + chr(39), line
            )
            if m:
...
def extract_structure(content):
    components = []
    if "split-grid" in content:
        components.append("split-grid")
    if "structured-list" in content:
        components.append("structured-list")
    if "dense-table" in content:
        components.append("dense-table")
    if "irab-box" in content:
        components.append("irab-box")
    if "exam-question" in content:
        components.append("exam-question")
    if "poem-container" in content:
        components.append("poem-container")
    return components
...
def main():
    ...
    for f in files:
        ...
        if "highlight-red" in content:
            stats["color_usage"]["highlight-red"] += 1
        if "highlight-blue" in content:
            stats["color_usage"]["highlight-blue"] += 1
        if "text-accent" in content:
            stats["color_usage"]["text-accent"] += 1
