...
for file in pages_files:
    ...
    # Fallback to filename parsing if lesson_number is missing or empty
    if not lesson_number:
        m = re.match(r'^([0-9]+)\.[0-9]+_', basename)
        if m:
            lesson_number = str(int(m.group(1))) # e.g. "04" -> "4"
            if basename.startswith('98.'):
                lesson_number = None # Answers don't have a lesson number
    ...
    if not is_preamble:
        toc_entries.append({
            ...
            'arabic_page': to_arabic_indic(logical_page)
        })
        logical_page += 1
...
print("Logical pages completely synced and TOC dashes fixed.")
