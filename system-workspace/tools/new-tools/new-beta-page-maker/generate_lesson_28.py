...
T_BLOCK = re.sub(
    r'<p class="mt-1mm text-accent">\s*\[CONTENT_TEXT\]\s*</p>', "[CONTENT_TEXT]", T_BLOCK
)
...
""",
    ...
    {"type": "BLOCK", "id": "b28007_title", "title": "أَنْوَاعُ الاسْتِعَارَةِ", "content": ""},
    {
        "type": "SPLIT",
        "id": "b28007",
    ...
    {"type": "BLOCK", "id": "b28008_title", "title": "التَّشْخِيصُ وَالتَّجْسِيمُ", "content": ""},
    {
        "type": "SPLIT",
        "id": "b28008",
    ...
    {"type": "BLOCK", "id": "b28012_title", "title": "٣- التَّحْسِينُ وَالتَّقْبِيحُ", "content": ""},
    {
        "type": "SPLIT",
        "id": "b28012",
    ...
    {"type": "BLOCK", "id": "b28013_title", "title": "٤- الوَصْفُ وَالإِيحَاءُ", "content": ""},
    {
        "type": "SPLIT",
        "id": "b28013",
    ...
    {"type": "BLOCK", "id": "b28014_title", "title": "٥- إِضْفَاءُ النَّفْسِيَّةِ وَالرَّمْزُ", "content": ""},
    {
        "type": "SPLIT",
        "id": "b28014",
...
def render_block(block):
    ...
    if block["type"] == "HEADER":
        ...
        t = re.sub(
            r'<div class="benefit-box[^>]*>.*?\[BENEFIT_TITLE\].*?</div>', "", t, flags=re.DOTALL
        )
        ...
        for item in items:
            chips_html += (
                f'<span class="bg-grey-lighter p-2mm rounded border-light">{item.strip()}</span>'
            )
        ...
        if 'id="' not in t and block.get("id"):
            t = t.replace(
                ...
                f'<section class="split-grid" id="{block.get("id", "")}">',
            )
        ...
        if 'id="' not in t and block.get("id"):
            t = t.replace(
                ...
                f'<section class="poem-container" id="{block.get("id", "")}">',
            )
    ...
    for i in range(1, len(blocks)):
        ...
        try:
            result_json = subprocess.check_output(
                [sys.executable, "Jules-workspace/verify_layout.py", temp_filename],
