...
def clean_block(content):
    content = re.sub(
        r'<div class="benefit-box">\s*<strong> \[BENEFIT_TITLE\]:</strong> \[BENEFIT_TEXT\]\s*</div>',
        "",
        content,
    )
    content = re.sub(
        r'<hr class="separator-dashed">\s*<div class="benefit-box">\s*<strong> \[NOTE_TITLE\]:</strong> \[NOTE_TEXT\]\s*</div>',
        "",
        content,
    )
    return content
...
def main():
    ...
    # 1. Header
    header = (
        t_header.replace("[LESSON_NUMBER]", "18")
    ...
    chips_html = "".join(
        [f'<span class="bg-grey-lighter rounded p-1mm">{item}</span>\n' for item in items]
    )
    ...
    block3_wrapper = re.sub(
        r'<p class="mt-1mm text-accent">.*?</p>', block3_content, block3_wrapper, flags=re.DOTALL
    )
    ...
    rows_html = "".join(
        [
            t_table_row.replace("[CELL_1]", r[0])
            .replace("[CELL_2]", r[1])
            .replace("[CELL_3]", r[2])
            for r in rows_data
        ]
    ...
    block4 = (
        t_table.replace("[TABLE_TITLE]", "جَدْوَلُ الْقَاعِدَةِ الْعَامَّةِ")
    ...
    # 5. Split
    block5_title = t_block.replace("[BLOCK_TITLE]", "أَمْثِلَةٌ تَطْبِيقِيَّةٌ مُفَصَّلَةٌ").replace(
        "[CONTENT_TEXT]", "إِلَيْكَ تَفْصِيلُ الْأَمْثِلَةِ وَتَعْلِيلِهَا:"
    )
    content_blocks.append(clean_block(block5_title))

    left_items = [
        "١. <span class='highlight-red'>تَئِن</span>",
        "٢. <span class='highlight-red'>الْبَأْس</span>",
        "٣. <span class='highlight-red'>سَأَلَ</span>",
    ]
    right_items = [
        "١. هَمْزَةٌ مُتَوَسِّطَةٌ <span class='highlight-red'>مَكْسُورَةٌ</span> سُبِقَتْ <span class='highlight-blue'>بِفَتْحٍ</span>. (الْكَسْرُ أَقْوَى مِنَ الْفَتْحِ -> نَبْرَة).",
        "٢. هَمْزَةٌ مُتَوَسِّطَةٌ <span class='highlight-red'>سَاكِنَةٌ</span> سُبِقَتْ <span class='highlight-blue'>بِفَتْحٍ</span>. (الْفَتْحُ أَقْوَى مِنَ السُّكُونِ -> أَلِف).",
        "٣. هَمْزَةٌ مُتَوَسِّطَةٌ <span class='highlight-red'>مَفْتُوحَةٌ</span> سُبِقَتْ <span class='highlight-blue'>بِفَتْحٍ</span>. (تَمَاثَلَتِ الْحَرَكَتَانِ -> أَلِف).",
    ]

    left_ul = (
        '<ul class="structured-list">'
        + "".join(
            [t_list_item.replace("[MARKER]", "•").replace("[CONTENT]", i) for i in left_items]
        )
        + "</ul>"
    )
    right_ul = (
        '<ul class="structured-list">'
        + "".join(
            [t_list_item.replace("[MARKER]", "•").replace("[CONTENT]", i) for i in right_items]
        )
        + "</ul>"
    )

    block5 = (
        t_split.replace("[LEFT_TITLE]", "الْكَلِمَاتُ")
        .replace("[LEFT_CONTENT]", left_ul)
        .replace("[RIGHT_TITLE]", "التَّحْلِيلُ وَالتَّعْلِيلُ")
        .replace("[RIGHT_CONTENT]", right_ul)
    )
    content_blocks.append(block5)

    # 6. Exceptions Header
    block6 = t_block.replace("[BLOCK_TITLE]", "الْحَالَاتُ الشَّاذَّةُ").replace(
        "[CONTENT_TEXT]",
        "تَشِذُّ عَنِ الْقَاعِدَةِ الْعَامَّةِ حَالَتَانِ رَئِيسِيَّتَانِ، حَيْثُ لَا نَنْظُرُ إِلَى قُوَّةِ الْحَرَكَاتِ بَلْ نَتَّبِعُ قَاعِدَةً خَاصَّةً.",
    )
    ...
    block7 = (
        t_split.replace("[LEFT_TITLE]", "الْهَمْزَةُ عَلَى السَّطْرِ")
        .replace("[LEFT_CONTENT]", f'<p class="mt-1mm text-accent">{left_body}</p>')
        .replace("[RIGHT_TITLE]", "الْهَمْزَةُ عَلَى النَّبْرَةِ")
        .replace("[RIGHT_CONTENT]", f'<p class="mt-1mm text-accent">{right_body}</p>')
    )
    content_blocks.append(block7)

    # 8. Benefit
    block8 = t_benefit.replace("[TIP_TITLE]", "فَائِدَةٌ هَامَّةٌ").replace(
        "[TIP_TEXT]",
    ...
    # 9. Exam
    block9 = (
        t_exam.replace("[TOPIC]", "الهمزة المتوسطة")
        .replace(
            "[QUESTION_TEXT]",
            'عَلِّلْ كِتَابَةَ الْهَمْزَةِ فِي كَلِمَةِ (<span class="highlight-red">مُؤْمِن</span>) وَفْقَ الْقَاعِدَةِ الْعَامَّةِ.',
            1,
        )
        .replace(
            "[QUESTION_TEXT]",
            'عَلِّلْ كِتَابَةَ الْهَمْزَةِ فِي كَلِمَةِ (<span class="highlight-red">بِيْئَة</span>) وَاذْكُرْ هَلْ هِيَ حَالَةٌ قِيَاسِيَّةٌ أَمْ شَاذَّةٌ.',
            1,
        )
    )
    ...
    # Final Assembly
    full_content = "\n".join(content_blocks)
    final_html = t_base.replace(
        "<!-- INJECT_CONTENT_HERE -->",
        t_wrapper.replace("<!-- INJECT_CONTENT_HERE -->", full_content),
    )
