...
def verify_content(html_content):
    ...
    try:
        result = subprocess.run(
            ["python3", VERIFY_SCRIPT, temp_file], capture_output=True, text=True
        )
...
def get_blocks():
    ...
    b3 = b3.replace(
        "[TIP_TEXT]",
        "بِمَقْدُورِ الطَّالِبِ اعْتِمَادُ الْقَالِبِ النَّظَرِيِّ الْآتِي فِي إِجَابَتِهِ، حِينَمَا يُسْأَلُ عَنْ دَوْرِ اسْتِعْمَالِ الْجُمْلَةِ الْاسْمِيَّةِ فِي خِدْمَةِ الْمَعْنَى. يَنْبَغِي لِلطَّالِبِ أَنْ يُشِيرَ فِي إِجَابَتِهِ إِلَى الْمَعَانِي الَّتِي أَرَادَ الشَّاعِرُ أَنْ يُؤَكِّدَ ثَبَاتَهَا وَاسْتِقْرَارَهَا وَدَيْمُومَتَهَا مِنْ خِلَالِ اسْتِعْمَالِ الْجُمْلَةِ الْاسْمِيَّةِ، أَيْ؛ يَجِبُ رَبْطُ الثَّبَاتِ وَالِاسْتِقْرَارِ وَالدَّيْمُومَةِ بِالْمَعْنَى.",
    )
    blocks.append(b3)
    ...
    r1 = (
        row_t.replace("[CELL_1]", "<strong>الْجُمْلَةُ الْاسْمِيَّةُ</strong>")
        .replace("[CELL_2]", "الثَّبَاتُ، الِاسْتِقْرَارُ، الدَّيْمُومَةُ")
        .replace("[CELL_3]", "تَأْكِيدُ صِفَةٍ مُلَازِمَةٍ، أَوْ حَالَةٍ شُعُورِيَّةٍ دَائِمَةٍ لَا تَتَبَدَّلُ.")
    )
    r2 = (
        row_t.replace("[CELL_1]", "<strong>الْجُمْلَةُ الْفِعْلِيَّةُ</strong>")
        .replace("[CELL_2]", "التَّغَيُّرُ، الْحَرَكَةُ، الْحَيَوِيَّةُ")
        .replace("[CELL_3]", "عَرْضُ الْأَحْدَاثِ فِي سِيَاقٍ حَرَكِيٍّ مُتَغَيِّرٍ وَمُتَجَدِّدٍ.")
    )
    ...
    q2_text = (
        "مَا الْفَرْقُ بَيْنَ وَظِيفَةِ الْجُمْلَةِ الْاسْمِيَّةِ وَوَظِيفَةِ الْجُمْلَةِ الْفِعْلِيَّةِ مِنْ حَيْثُ الدَّلَالَةُ عَلَى الزَّمَنِ وَالْحَرَكَةِ؟"
    )
...
def main():
    ...
    if page_2_blocks:
        ...
        b1 = box.replace("[WORD]", "لَهُمْ").replace(
            "[PARSING_DETAILS]",
            "اللَّامُ حَرْفُ جَرٍّ، وَ(هُمْ) ضَمِيرٌ مُتَّصِلٌ مَبْنِيٌّ فِي مَحَلِّ جَرٍّ بِحَرْفِ الْجَرِّ، وَالْجَارُّ وَالْمَجْرُورُ مُتَعَلِّقَانِ بِخَبَرٍ مُقَدَّمٍ مَحْذُوفٍ.",
        )
        b2 = box.replace("[WORD]", "أَثَرٌ").replace(
            "[PARSING_DETAILS]", "مُبْتَدَأٌ مُؤَخَّرٌ مَرْفُوعٌ وَعَلَامَةُ رَفْعِهِ الضَّمَّةُ الظَّاهِرَةُ عَلَى آخِرِهِ."
        )
        ...
        box = get_template("TEMPLATE_C_IRAB_BOX.html")
        b3 = box.replace("[WORD]", "الرِّزْقُ").replace(
            "[PARSING_DETAILS]", "مُبْتَدَأٌ مَرْفُوعٌ وَعَلَامَةُ رَفْعِهِ الضَّمَّةُ الظَّاهِرَةُ عَلَى آخِرِهِ."
        )
        b4 = box.replace("[WORD]", "جَمَدْ").replace(
            "[PARSING_DETAILS]",
            "فِعْلٌ مَاضٍ مَبْنِيٌّ عَلَى الْفَتْحِ الظَّاهِرِ، وَالْفَاعِلُ ضَمِيرٌ مُسْتَتِرٌ تَقْدِيرُهُ (هُوَ)، وَالْجُمْلَةُ الْفِعْلِيَّةُ فِي مَحَلِّ رَفْعٍ خَبَرُ الْمُبْتَدَأِ.",
        )
        ...
        summary = summary.replace(
            "[BENEFIT_TEXT]",
            "الْجُمْلَةُ الْاسْمِيَّةُ تُفِيدُ الثَّبَاتَ وَالِاسْتِقْرَارَ، بَيْنَمَا تُفِيدُ الْجُمْلَةُ الْفِعْلِيَّةُ التَّجَدُّدَ وَالْحُدُوثَ وَالْحَرَكَةَ.",
        )
